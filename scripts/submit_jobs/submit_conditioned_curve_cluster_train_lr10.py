...
CONFIG_PATH = "configs/parametric_edge/laion_conditioned_curve_pretrain_cluster_v3_2gpu_lr10.yaml"
SBATCH_PATH = f"{CLUSTER_REPO}/cluster_tasks/train_conditioned_curve_dab_v3_2gpu_lr10.sbatch"
...
def build_sbatch() -> str:
    return dedent(
        f"""\
        #!/usr/bin/env bash
        #SBATCH -J cond-curve-dab-v3-2gpu-lr10
        #SBATCH -p gbunchQ
        #SBATCH --gres=gpu:2
        #SBATCH -c 32
        #SBATCH --mem=64G
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Print or submit the 2-GPU cluster conditioned-curve DAB lr/10 successor job.")
