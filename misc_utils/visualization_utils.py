...
def draw_curve(ax, control_points: np.ndarray, width: int, height: int, color: str, linewidth: float = 2.0) -> None:
...
def render_curve_grid(images: torch.Tensor, targets: List[dict], predictions: List[torch.Tensor], output_path: Path, titles: Iterable[str] = ('Input', 'Target Curves', 'Predicted Curves')) -> None:
